...
// The generated worker is maintained as a readable shell script (src/main/qcdelegate.sh)
// and embedded here base64-encoded — its heavy ${...} expansions, backslashes, and ANSI
// escapes make a TS template literal a maintenance hazard. It reads its target model
// from ~/.quadclaude/delegation-model (so repointing delegation never regenerates the
// script), warms past local-model cold starts, measures exactly what it changed, and
// appends a structured event to ~/.quadclaude/events.jsonl for the app to roll up.
